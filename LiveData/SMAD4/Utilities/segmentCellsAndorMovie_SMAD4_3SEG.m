...
function segmentCellsAndorMovie_SMAD4_3SEG(direc,pos,chan,paramfile,outfile)
 ...
 disp(['Reading ii=',num2str(ii)])
   ...
   % h5file = geth5nameMask3(filename); %ilastik update defaults to prefix "mask3" 3/13/18 jkm
    if exist(h5file,'file')
        usemask = 1;
        [masks,smadmask,bgMask] = readIlastikFileNucMemBG(h5file); %Load masks corresponding to this file
...
%         fimg2 = medfilt2(presubBackground_provided(fimg,2));
        ...
        % ---------------------
        fgMask = imclose(smadmask(:,:,jj),strel('disk',10));
...
%             imshow(cat(3,cytmaskprov,Rawnucplot,fgMask),[])
%             title('Old mask')
%             pause()
%%
